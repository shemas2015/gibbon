...
if (isActionAccessible($guid, $connection2, '/modules/User Admin/user_documents.php') == false) {
    ...
    if (empty($gibbonPersonID) || reset($_SESSION)["gibbonPersonID"] !== $gibbonPersonID)  {
        ...
        if (empty($values)) {
            ...
            $form = Form::create('addUser', $session->get('absoluteURL').'/modules/'.$session->get('module').'/user_documents_editProcess.php?gibbonPersonID='.$gibbonPersonID.'&search='.$search);
            ...
            $row = $form->addRow();
                ...
                $row->addTextField('surname')->readOnly();

            $row = $form->addRow();
                $row->addLabel('firstName', __('First Name'))->description(__('First name as shown in ID documents.'));
                $row->addTextField('firstName')->readOnly();

            $row = $form->addRow();
                $row->addLabel('preferredName', __('Preferred Name'))->description(__('Most common name, alias, nickname, etc.'));
                $row->addTextField('preferredName')->readOnly();


            $row = $form->addRow();
                $row->addLabel('gender', __('Gender'));
                $row->addSelectGender('gender')->readOnly();

            $row = $form->addRow();
                $row->addLabel('dob', __('Date of Birth'));
                $row->addDate('dob')->disabled();
